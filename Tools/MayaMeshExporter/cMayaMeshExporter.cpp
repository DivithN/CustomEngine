...
namespace
{
	...
	MStatus ProcessAllObjects( std::map<std::string, sVertexInfo>& o_uniqueVertices, std::vector<sTriangle>& o_triangles,
		std::vector<MObject>& o_shadingGroups , MMatrix& o_transformMatrix, std::vector<MTransformationMatrix>& io_animationKeyFrames);
	...
	MStatus ProcessSingleDagNode( const MDagPath& i_dagPath,
		...
		std::vector<MObject>& io_shadingGroups, std::map<std::string, size_t>& io_map_shadingGroupNamesToIndices, MMatrix& io_transformMatrix,
		std::vector<MTransformationMatrix>& io_animationKeyFrames);
	MStatus WriteMeshToFile( const MString& i_fileName, const std::vector<sVertexInfo>& i_vertexArray, const std::vector<size_t>& i_indexArray,
		const std::vector<sMaterialInfo>& i_materialInfo, MMatrix& i_transformMatrix, const std::vector<MTransformationMatrix>& i_animationKeyframes);

	MStatus GetTransformAnimation(const MDagPath& i_dagPath, std::vector<MTransformationMatrix>& o_keyframes);
}
...
MStatus eae6320::cMayaMeshExporter::writer( const MFileObject& i_file, const MString& i_options, FileAccessMode i_mode )
{
	...
	MMatrix MeshTransform = MMatrix::identity;

	std::vector<MTransformationMatrix> animationKeyframes;
	...
	std::vector<sMaterialInfo> materialInfo;
	{
		...
		std::vector<MObject> shadingGroups;
		{
			...
			{
				if ( !( status = ProcessAllObjects( uniqueVertices, triangles, shadingGroups, MeshTransform, animationKeyframes) ) )
				{
					return status;
				}
			}
			...
			{
				if ( !( status = ProcessSelectedObjects( uniqueVertices, triangles, shadingGroups, MeshTransform, animationKeyframes) ) )
				{
					return status;
				}
			}
	...
	// Write the mesh data to the requested file
	{
		...
		return WriteMeshToFile( filePath, vertexArray, indexArray, materialInfo, MeshTransform, animationKeyframes);
	}
}
...
namespace
{
		...
		std::vector<MObject>& o_shadingGroups, MMatrix& o_transformMatrix, std::vector<MTransformationMatrix>& io_animationKeyFrames)
	{
		...
		for ( MItDag i( MItDag::kDepthFirst, MFn::kMesh ); !i.isDone(); i.next() )
		{
			MDagPath dagPath;
			i.getPath( dagPath );
			if ( !( status = ProcessSingleDagNode( dagPath, o_uniqueVertices, o_triangles, o_shadingGroups, map_shadingGroupNamesToIndices , o_transformMatrix, io_animationKeyFrames) ) )
			{
				return status;
			}
		}
	...
	MStatus ProcessSingleDagNode( const MDagPath& i_dagPath,
		std::map<std::string, sVertexInfo>& io_uniqueVertices, std::vector<sTriangle>& io_triangles,
		std::vector<MObject>& io_shadingGroups, std::map<std::string, size_t>& io_map_shadingGroupNamesToIndices , MMatrix& io_transformMatrix, std::vector<MTransformationMatrix>& io_animationKeyFrames)
	{
		...
		// Get mesh Transform
		io_transformMatrix = i_dagPath.inclusiveMatrix();

		// Get animation keyframes
		GetTransformAnimation(i_dagPath, io_animationKeyFrames);

		// Get a list of the positions
		MPointArray positions;
		{
			const auto status = mesh.getPoints( positions, MSpace::kObject );
			if ( !status )
			{
				MGlobal::displayError( status.errorString() );
				return status;
		...
		{
			const auto status = mesh.getNormals( normals, MSpace::kObject );
			if ( !status )
			{
				MGlobal::displayError( status.errorString() );
				return status;
	...
	MStatus WriteMeshToFile( const MString& i_fileName, const std::vector<sVertexInfo>& i_vertexArray, const std::vector<size_t>& i_indexArray,
		const std::vector<sMaterialInfo>& i_materialInfo, MMatrix& i_meshTransform, const std::vector<MTransformationMatrix>& i_animationKeyframes)
	{
		...
		MTransformationMatrix meshTransformMatrix(i_meshTransform);
		MVector translation = meshTransformMatrix.getTranslation(MSpace::kWorld);

		// get rotation as Euler angles
		MQuaternion rotation = meshTransformMatrix.eulerRotation().asQuaternion();

		// get scale
		double scale[3];
		meshTransformMatrix.getScale(scale, MSpace::kWorld);
...
//			fout << std::fixed;
				...
				"{" "\n";
			{
				...
				fout << "\tmeshTransform =\n\t{\n";
				fout << "\t\ttranslation = {" << static_cast<float>(translation.x) << ", " << static_cast<float>(translation.y) << ", " << static_cast<float>(translation.z) << "},\n";
				fout << "\t\trotation = {" << static_cast<float>(rotation.x) << ", " << static_cast<float>(rotation.y) << ", " << static_cast<float>(rotation.z) 
											<< ", " << static_cast<float>(rotation.w) << "},\n";
				fout << "\t\tscale = {" << static_cast<float>(scale[0]) << ", " << static_cast<float>(scale[1]) << ", " << static_cast<float>(scale[2]) << "},\n";
				fout << "\t},\n";

				//animation part

				fout << "\tanimation =\n\t{\n";
				for (size_t i = 0; i < i_animationKeyframes.size(); ++i)
				{
					MVector pos = i_animationKeyframes[i].getTranslation(MSpace::kWorld);
					MQuaternion rot = i_animationKeyframes[i].rotation();
					fout << "\t\t{ frame = " << i << ", ";
					fout << "translation = {" << pos.x << ", " << pos.y << ", " << pos.z << "}, ";
					fout << "rotation = {" << rot.x << ", " << rot.y << ", " << rot.z << ", " << rot.w << "} },\n";
				}
				fout << "\t},\n";
				...
				fout << "\tverticies =\n\t{\n";
				for (unsigned int i = 0; i < i_vertexArray.size(); i++)
				{
					fout << "\t\t{" << i_vertexArray[i].vertex.x << ", " << i_vertexArray[i].vertex.y << ", " << i_vertexArray[i].vertex.z << ", "<<i_vertexArray[i].vertex.r<<", " << i_vertexArray[i].vertex.g << ", " << i_vertexArray[i].vertex.b<<", " << i_vertexArray[i].vertex.a << "},\n";
				}
				fout << "\t},\n";
	...
	MStatus GetTransformAnimation(const MDagPath& i_dagPath, std::vector<MTransformationMatrix>& o_keyframes)
	{
		...
		MFnDependencyNode depNode(i_dagPath.transform(&status));
		if (!status)
		{
		...
		for (MTime time = startTime; time <= endTime; time += 1.0)
		{
			MGlobal::viewFrame(time);
			MMatrix mat = i_dagPath.inclusiveMatrix(&status);
			if (!status)
			{
				MGlobal::displayError("Failed to get matrix at time ");
				return status;
			}
			o_keyframes.push_back(MTransformationMatrix(mat));
		}
